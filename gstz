...
do
    ...
    do
        ...
        if not v7 then
            v4.GomesCfgGen = (tonumber(v4.GomesCfgGen) or 0) + 1
        end
    ...
    if type(unload) == "function" then
        pcall(unload)
    elseif type(v4.GomesUnload) == "function" then
        local GomesUnloadUid = v4.GomesUnloadUid
        local v12 = GomesUnloadUid == nil or str == tostring(GomesUnloadUid)

        if GomesUnloadUid == nil then
            for k, v in pairs(GomesHub.slots) do
        ...
        if v12 then
            local GomesUnload = v4.GomesUnload

            if str == tostring(GomesUnloadUid or str) then
                v4.GomesUnload = nil
                v4.GomesUnloadUid = nil
            end
...
local t1 = {
	...
	Status = "preview",
	Game = "Steal an Egg",
	Discord = "https://discord.gg/nEsMReMb",
	Website = "",
	Changelog = "",
	Author = "Gomes (@gomes_scripts.gg)",
...
t1.LogoFile = "Gomes" .. "/logo.png"
t1.LogoFileLight = "Gomes" .. "/logo-light.png"
local n1 = 620
local n2 = 430
local n3 = 152
local Players = game:GetService("Players")
...
do
    ...
    do
        ...
        do
            ...
            t1.ConfigFile = (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json"
            t2 = {
				Dark = {
					...
					Gomes = Color3.fromRGB(246, 242, 234)
				},
				Light = {
            ...
            if PH_UI then
                local GomesUnloadUid = (getgenv and getgenv() or _G).GomesUnloadUid

                if GomesUnloadUid == nil or str == tostring(GomesUnloadUid) then
                    PH_UI:Destroy()
                end
            end
    ...
    local function v271(p58, p59)
        ...
        local v602 = "Gomes" .. "/" .. v26(t1.Game)

        if type(makefolder) == "function" then
            pcall(makefolder, v602)
        end

        local v603 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/cache"

        if type(makefolder) == "function" then
            pcall(makefolder, v603)
        end

        local v604 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v272(p60)
        ...
        if not v271((("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json", result) then
            v271((("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. str .. "-config.json", result)
        end
    end
    ...
    local function v276()
        ...
        if type(listfiles) == "function" then
            local ok, result = pcall(listfiles, ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs")
    ...
    function v278(p68)
        ...
        local v647 = v271
        local v648 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v279(p69, p70)
        ...
        local v654 = v275
        local v655 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v280(p71)
        ...
        local v661, v662 = v275((("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json")

        if not v661 then
            v661, v662 = v275((("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. str .. "-config.json")
        end

        if not v661 then
            local v663 = v275
            local v664 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
        ...
        if not v661 then
            local t85 = {
				("Gomes" .. "/" .. v26(t1.Game)) .. "/config.json",
				"Gomes" .. "/config.json",
				"Gomes" .. "/" .. t1.Game .. "/config.json",
				("Gomes" .. "/" .. v26(t1.Game)) .. "/Gomes.json",
				"Gomes" .. "/Gomes.json",
				"Gomes.json"
			}
        ...
        if v662 ~= (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json" then
            v272(true)
        end
...
v282(v295, "ReturnSpeed", "Return speed", "Studs/s while returning to your safe zone", 50, 1300, 300, " studs/s")
v263(v295, "Targeting")
...
v263(v298, "Live Eggs")
v287(v298, "LiveEggRarity", "Minimum rarity", "only show eggs at or above this rarity in Live Eggs", {"Any", unpack(t6)}, "Any", false)
-- LiveEggTop3 removido: Live Eggs mostra todos os ovos encontrados.
v281(v298, "LiveEggUseESP", "Use ESP filter", "reuse the current Egg ESP filter when building Live Eggs", false)
v281(v298, "LiveEggAutoOpen", "Auto-open on high rarity", "opens Live Eggs automatically when a Mythic+ egg appears", false)
v281(v298, "LiveEggAlert", "High rarity alert", "shows a small alert when a Mythic+ live egg appears", true)
v281(v298, "LiveEggCompact", "Compact list", "reduces each egg card height so more eggs fit on screen", false)
v263(v298, "Auto Steal approach")
v281(v298, "EggHoverApproach", "7 studs above egg", "when enabled, only the trip to the egg targets a point 7 studs above it; returning stays normal", false)
v287(v298, "EggApproachMethod", "Approach method", "movement used only while going to the egg", {"Speed", "Fling"}, "Speed", false)
...
v289(v300, "SaveCfgAs", "Save config", "writes Gomes/.../configs/<name>.json", "Save", function()
    v278(t9.CfgSaveName)
end, true)
...
v325.UI = t149
v324.GomesUI = type(v324.GomesUI) == "table" and v324.GomesUI or {}
v324.GomesUI[str] = t149
...
(function()
    ...
    local function v1144()
        ...
        if v1494 then
            for _, child in ipairs(workspace:GetChildren()) do
                if child.Name == "GomesSupport" or child.Name == "Hub45Support" then
                    pcall(function()
                        child:Destroy()
                    end)
                end
    ...
    local function v1151()
        ...
        if u1128 and typeof(u1127) == "Vector3" then
            local returning = t216 and (t216.carrying == true or t216.state == "Return" or t216.state == "Bank")
            if returning then
                return math.clamp(tonumber(t9.ReturnSpeed) or 300, 50, 1300)
            end
    ...
    local function v1154(p178, p179)
        ...
        if u1128 and u1127 then
            ...
            -- Optional approach offset: applies only while travelling to the egg.
            -- Return/Bank keeps the original target untouched.
            if t9.EggHoverApproach and t216 and t216.state == "GoEgg" and not t216.carrying then
                v1551 = v1551 + Vector3.new(0, 7, 0)
            end

            local v1553 = v1551 - p179.Position
    ...
    local function v1182(p218)
        ...
        if v1682 then
            if v1685 then
                ...
                -- Fling-style approach is restricted to the outbound GoEgg phase.
                -- Returning to the safe zone continues through the existing movement engine.
                if t9.EggHoverApproach and t9.EggApproachMethod == "Fling" and t216 and t216.state == "GoEgg" and not t216.carrying and u1127 then
                    local vFlingTarget = u1127 + Vector3.new(0, 7, 0)
                    local vFlingDelta = vFlingTarget - v1685.Position
                    if vFlingDelta.Magnitude > 0.05 then
                        local vFlingSpeed = math.clamp(tonumber(t9.StealSpeed) or 300, 50, 1300)
                        v1688 = vFlingDelta.Unit * vFlingSpeed
                    end
                end
    ...
    local u1195 = (function()
        ...
        local function v1810(p269, p270, p271)
            ...
            if p271 and p271.bytes then
                local v2996 = "Gomes" .. tostring(math.floor(os.clock() * 1000000)) .. tostring(math.random(100000, 999999))
        ...
        local function v1811(p272)
            ...
            local str9 = tostring(p272.title or "Gomes")
            local v3014
            local g3028
            local str10
            local s17
    ...
    u1134 = (function()
        ...
        local function v2119()
            local GomesEsp = workspace:FindFirstChild("GomesEsp")

            if GomesEsp and GomesEsp:IsA("Folder") then
                t218.folder = GomesEsp
        ...
        local function v2120()
            ...
            if PlayerGui then
                local v3251 = PlayerGui:FindFirstChild(v49) or PlayerGui:FindFirstChild("GomesWorldGui")
        ...
        local function v2143(p370)
            if p370 then
                ...
                elseif t218.world and p370:IsDescendantOf(t218.world) then
                    ...
                    v3530 = p370Name == "GomesSupport" or (p370Name == "Hub45Support" or (p370Name == "GomesWorldGui" or (p370Name == v49 or p370Name == v48)))
                end
    ...
    local u1230 = (function()
        ...
        v2192.GomesHopUsed = type(v2192.GomesHopUsed) == "table" and v2192.GomesHopUsed or {}
        v2192.GomesHopRing = type(v2192.GomesHopRing) == "table" and v2192.GomesHopRing or {}
        v2192.GomesHopCache = type(v2192.GomesHopCache) == "table" and v2192.GomesHopCache or {}
        if type(v2192.GomesHopUntil) == "number" then
            ...
            if math.abs(v2195 - v2192.GomesHopUntil) < 10 then
            end
        end
        ...
        local function v2197(p390)
            ...
            local GomesHopRing = v2192.GomesHopRing

            for i = #GomesHopRing, 1, -1 do
                if p390 == GomesHopRing[i] then
                    table.remove(GomesHopRing, i)
                end
            end

            GomesHopRing[#GomesHopRing + 1] = p390

            while #GomesHopRing > 48 do
                table.remove(GomesHopRing, 1)
            end
        end
        local function v2198(p391)
            ...
            local g3859
            local v3858
            for i = 1, #GomesHopRing do
                if str28 == GomesHopRing[i] then
                    v3858 = true
                    g3859 = true
                end
            ...
            local v3860 = v2192.GomesHopUsed[str28]
            if type(v3860) ~= "number" then
                return false
            end
            if os.time() - v3860 > 2100 then
                v2192.GomesHopUsed[str28] = nil
        ...
        local function v2199()
            ...
            local ok54, result54 = pcall(readfile, (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-used.json")
            ...
            if type(data.jobs) == "table" then
                for k, v in pairs(data.jobs) do
                    ...
                    if v3867 and timestamp - v3867 <= 2100 and v3867 > (tonumber(v2192.GomesHopUsed[str29]) or 0) then
                        v2192.GomesHopUsed[str29] = v3867
                    end
                end
            end
            if type(data.ring) == "table" then
                ...
                for i = 1, #v2192.GomesHopRing do
                    local v3874 = v2192.GomesHopRing[i]
                ...
                v2192.GomesHopRing = t282

                while #v2192.GomesHopRing > 48 do
                    table.remove(v2192.GomesHopRing, 1)
                end
            end
        end
        local function v2200()
            ...
            local v3875 = "Gomes" .. "/" .. v26(t1.Game)

            if type(makefolder) == "function" then
                pcall(makefolder, v3875)
            end

            local v3876 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/cache"

            if type(makefolder) == "function" then
                pcall(makefolder, v3876)
            end

            local v3877 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
            ...
            for k, v in pairs(v2192.GomesHopUsed) do
            ...
            if ok55 and type(result55) == "string" then
                pcall(writefile, (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-used.json", result55)
            end
        end
        v2199()
        local str30 = tostring(tostring(game.JobId) or "")
        if str30 ~= "" then
            v2192.GomesHopUsed[str30] = os.time()
            v2197(str30)
        end
        v2200()
        ...
        local function v2205(p395)
            local GomesHopCache = v2192.GomesHopCache

            if type(GomesHopCache) ~= "table" or (GomesHopCache.key ~= v2203() or type(GomesHopCache.list) ~= "table" or #GomesHopCache.list == 0) then
                GomesHopCache = nil

                if type(readfile) == "function" then
                    local ok58, result58 = pcall(readfile, (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-list.json")
                    ...
                    if ok58 and type(result58) == "string" and result58 ~= "" then
                        ...
                        if type(data) == "table" and data.key == v2203() and type(data.list) == "table" and #data.list > 0 then
                            GomesHopCache = data
                            v2192.GomesHopCache = data
                        end
                    end
                end
            end

            if type(GomesHopCache) ~= "table" or type(GomesHopCache.list) ~= "table" or #GomesHopCache.list == 0 then
                return
            end

            local v3913 = os.time() - (tonumber(GomesHopCache.at) or 0)

            if v3913 > 90 and not p395 then
                return
            end

            v2204(GomesHopCache.list)
        ...
        local function v2206(p396)
            ...
            if type(writefile) == "function" then
                ...
                local v3916 = "Gomes" .. "/" .. v26(t1.Game)

                if type(makefolder) == "function" then
                    pcall(makefolder, v3916)
                end

                local v3917 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/cache"

                if type(makefolder) == "function" then
                    pcall(makefolder, v3917)
                end

                local v3918 = ("Gomes" .. "/" .. v26(t1.Game)) .. "/configs"
                ...
                if ok59 and type(result59) == "string" then
                    pcall(writefile, (("Gomes" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-list.json", result59)
                end
            end
        end
        ...
        local function v2208()
            ...
            for i = math.max(1, #GomesHopRing - 7), #GomesHopRing do
                local v3939 = GomesHopRing[i]
        ...
        local function v2209()
            ...
            if str35 ~= "" then
                v2192.GomesHopUsed[str35] = os.time()
                v2197(str35)
            end
        ...
        local function v2211(p399)
            ...
            while u1117 and u2179 do
                ...
                v3984.GomesHopUntil = (not ok62 or type(result62) ~= "number") and 0 or math.max(0, result62)
                u2178 = true
    ...
    function u1132(p475)
        ...
        if u1127 and not u1116 then
            ...
            pcall(function()
                ...
                if u1128 and typeof(u1127) == "Vector3" then
                    local returning = t216 and (t216.carrying == true or t216.state == "Return" or t216.state == "Bank")
                    local v4575 = returning and (tonumber(t9.ReturnSpeed) or 300) or (tonumber(t9.StealSpeed) or 300)
    ...
    pcall(function()
        ...
        AutoStealTestNewGui.Name = "GomesAutoStealTestNew"
    ...
    v1238("ReturnSpeed", function(p485r)
        v1102("engine", "return speed", p485r)
    end)
    v1238("EggHoverApproach", function(p485h)
        v1102("engine", "7 studs above egg", p485h)
    end)
    v1238("EggApproachMethod", function(p485m)
        v1102("engine", "egg approach method", tostring(p485m))
    end)
    ...
    function u323()
        ...
        local ok103, result99, _, _ = pcall(function()
            ...
            if PlayerGui then
                local GomesWorldGui = PlayerGui:FindFirstChild("GomesWorldGui")

                if GomesWorldGui then
                    GomesWorldGui:Destroy()
                end
            end
        ...
        if type(v2668.GomesUI) == "table" then
            v2668.GomesUI[str] = nil
        end

        if type(v2668.GomesDumpByUser) == "table" then
            v2668.GomesDumpByUser[str] = nil
        end
        ...
        if v2668.GomesUnloadUid == str then
            v2668.GomesUnloadUid = nil
            v2668.GomesUnload = nil
        end
    ...
    v1244.unload = u323
    v1244.alive = true
    v1243.GomesUnloadUid = str
    function v1243.GomesUnload()
    ...
    local v1245 = v50()
    v51().dump = t149.GomesDump
    v1245.GomesDumpByUser = type(v1245.GomesDumpByUser) == "table" and v1245.GomesDumpByUser or {}
    v1245.GomesDumpByUser[str] = t149.GomesDump
    function v1245.GomesDump(...)
        local LocalPlayer3 = Players.LocalPlayer
        local str43 = tostring(LocalPlayer3 and LocalPlayer3.UserId or 0)
        local v2683 = v1245.GomesDumpByUser and v1245.GomesDumpByUser[str43]
    ...
    -- Live Eggs: painel separado, leve e atualizando com os ovos reais do campo.
    do
        local LiveEggsGui
        local LiveEggsPanel
        local LiveEggsList
        local LiveEggsStatus
        local LiveEggsButton
        local LiveEggsEggTab
        local LiveEggsPlotTab
        local LiveEggsEggList
        local LiveEggsPlotList
        local LiveEggsTopPets
        local LiveEggsCurrentTab = "Eggs"

        local function leFmt(n)
            n = tonumber(n) or 0
            local a = math.abs(n)
            ...
            if a >= 10 then return string.format("%.0f", n) end
            return string.format("%.1f", n)
        end

        local function leText(v)
        ...
        local function leName(t)
            if type(t) ~= "table" then return nil end
            return leText(t.DisplayName)
                or leText(t.Name)
                or leText(t.name)
                or leText(t.PetName)
                or leText(t.petName)
                or leText(t.AssetCategory)
                or leText(t.Category)
                or leText(t.category)
        end

        local function leEarn(t)
            if type(t) ~= "table" then return 0 end
            return tonumber(t.Earn)
                or tonumber(t.Earnings)
                or tonumber(t.Income)
                or tonumber(t["$/s"])
                or tonumber(t.Gen)
                or tonumber(t.Generation)
                or tonumber(t.Value)
                or tonumber(t.Price)
                or tonumber(t.earn)
                or 0
        ...
        local function leChance(t)
            if type(t) ~= "table" then return nil end
            return leText(t.Chance) or leText(t.chance) or leText(t.Percent) or leText(t.Weight)
        end

        local function leRarity(t)
            if type(t) ~= "table" then return "?" end
            local r = t.Rarity
            if type(r) == "table" then
                return leText(r.DisplayName) or leText(r.Name) or leText(r._id) or "?"
            end
            return leText(r) or leText(t.RarityName) or "?"
        end

        local function leIsPet(t, key)
            if type(t) ~= "table" then return false end
            local cat = string.lower(tostring(t.AssetCategory or t.Category or t.category or key or ""))
            if cat:find("pet", 1, true) then return true end
            if t.PetName or t.petName or t.PetId or t.petId then return true end
            local n = string.lower(tostring(t.DisplayName or t.Name or t.name or ""))
            return n ~= "" and (t.Earn ~= nil or t.Earnings ~= nil or t.Income ~= nil) and cat ~= "egg"
        end

        local function leIcon(cfg, cat, name)
            if not u1134 then return "" end
            local icon = ""
            pcall(function()
                if u1134.liveIcon then
                    local ok, result = pcall(u1134.liveIcon, cfg, cat, name)
                    if ok and type(result) == "string" and result ~= "" then
                        icon = result
                    end
                end
            ...
            if icon == "" then
                pcall(function()
                    if u1134.icon then
                        local ok, result = pcall(u1134.icon, cfg, cat)
                        if ok and type(result) == "string" and result ~= "" then
                            icon = result
                        end
                    end
        ...
        local function leCollectPets(root, out, seen, depth, key)
            if type(root) ~= "table" or depth > 7 then return end
            seen = seen or {}
            if seen[root] then return end
            seen[root] = true

            if leIsPet(root, key) then
                local name = leName(root)
                if name then
                    out[#out + 1] = {
                        name = name,
                        earn = leEarn(root),
                        chance = leChance(root),
                        rarity = leRarity(root),
                        cfg = root,
                        cat = root.AssetCategory or root.Category or root.category,
                        icon = ""
                    }
                end
            end
            ...
            for k, v in pairs(root) do
                if type(v) == "table" then
                    local ks = tostring(k)
                    local lower = string.lower(ks)
                    if lower:find("pet", 1, true)
                        or lower:find("reward", 1, true)
                        or lower:find("loot", 1, true)
                        or lower:find("drop", 1, true)
                        or lower:find("content", 1, true)
                        or lower:find("animal", 1, true) then
                        leCollectPets(v, out, seen, depth + 1, ks)
                    elseif depth < 3 then
                        leCollectPets(v, out, seen, depth + 1, ks)
                    end
                end
        ...
        local function leClear()
            if not LiveEggsList then return end
            for _, child in ipairs(LiveEggsList:GetChildren()) do
                child:Destroy()
            end
        end

        local function leRound(obj, radius)
        ...
        local function leStroke(obj, color, thickness)
        ...
        local function leMakePetRow(parent, pet, order)
            local row = Instance.new("Frame")
            row.Name = "Pet_" .. tostring(order)
            row.LayoutOrder = order
            row.Size = UDim2.new(1, -8, 0, 46)
            row.BackgroundColor3 = t3.fill
            ...
            icon.Image = pet.icon or ""
            icon.Parent = row

            local name = Instance.new("TextLabel")
            name.BackgroundTransparency = 1
            name.Position = UDim2.fromOffset(47, 4)
            name.Size = UDim2.new(1, -150, 0, 20)
            name.Font = t4.mid
            name.Text = tostring(pet.name)
            name.TextColor3 = t3.text
            name.TextSize = 12
            ...
            meta.Position = UDim2.fromOffset(47, 24)
            meta.Size = UDim2.new(1, -150, 0, 17)
            meta.Font = t4.body
            meta.Text = tostring(pet.rarity or "?") .. (pet.chance and (" · " .. tostring(pet.chance)) or "")
            meta.TextColor3 = t3.dim
            meta.TextSize = 10
            ...
            earn.AnchorPoint = Vector2.new(1, 0.5)
            earn.Position = UDim2.new(1, -8, 0.5, 0)
            earn.Size = UDim2.fromOffset(88, 30)
            earn.Font = t4.mid
            earn.Text = pet.earn > 0 and (leFmt(pet.earn) .. "/s") or "—"
            earn.TextColor3 = t3.accent
            earn.TextSize = 11
            earn.TextXAlignment = Enum.TextXAlignment.Right
            earn.Parent = row
        end

        local function leMakeEggRow(parent, egg, order)
            local cfg = egg.cfg
            local name = cfg and (cfg.DisplayName or egg.AssetCategory) or tostring(egg.AssetCategory or "Egg")
            local earn = v1193(egg, cfg)
            local rar = "?"
            ...
            card.Name = "Egg_" .. tostring(order)
            card.LayoutOrder = order
            card.Size = UDim2.new(1, -8, 0, t9.LiveEggCompact and 116 or 154)
            card.BackgroundColor3 = t3.card
            card.BorderSizePixel = 0
            card.Parent = LiveEggsList
            leRound(card, 11)
            leStroke(card, t3.line, 1)
            ...
            icon.ScaleType = Enum.ScaleType.Fit
            icon.Image = leIcon(cfg, egg.AssetCategory, name)
            icon.Parent = card

            local title = Instance.new("TextLabel")
            title.BackgroundTransparency = 1
            title.Position = UDim2.fromOffset(70, 8)
            title.Size = UDim2.new(1, -80, 0, 22)
            title.Font = t4.title
            title.Text = tostring(order) .. ". " .. tostring(name)
            title.TextColor3 = t3.text
            title.TextSize = 13
            ...
            info.Position = UDim2.fromOffset(70, 31)
            info.Size = UDim2.new(1, -80, 0, 22)
            info.Font = t4.body
            info.Text = leFmt(earn) .. "/s  ·  " .. rar .. "  ·  " .. tostring(egg.AreaId or "field")
            info.TextColor3 = t3.dim
            info.TextSize = 10
            ...
            petsFrame.Position = UDim2.fromOffset(8, 66)
            petsFrame.Size = UDim2.new(1, -16, 0, t9.LiveEggCompact and 46 or 80)
            petsFrame.Parent = card

            local layout = Instance.new("UIListLayout")
            layout.Padding = UDim.new(0, 4)
            layout.SortOrder = Enum.SortOrder.LayoutOrder
            layout.Parent = petsFrame

            local pets = {}
            pcall(function()
                leCollectPets(cfg, pets, nil, 0, egg.AssetCategory)
            end)

            table.sort(pets, function(a, b)
                if a.earn ~= b.earn then return a.earn > b.earn end
            ...
            for _, pet in ipairs(pets) do
                local key = string.lower(tostring(pet.name))
                if not unique[key] then
                    unique[key] = true
                    pet.icon = leIcon(pet.cfg, pet.cat, pet.name)
                    leMakePetRow(petsFrame, pet, shown + 1)
                    shown += 1
                end
            end
            ...
            if shown > 0 then
                local rowH = 46
                local gap = 4
                local totalH = shown * rowH + math.max(0, shown - 1) * gap
                petsFrame.Size = UDim2.new(1, -16, 0, totalH)
                card.Size = UDim2.new(1, -8, 0, 74 + totalH + 10)
            elseif t9.LiveEggCompact then
                card.Size = UDim2.new(1, -8, 0, 116)
            end

            if shown == 0 then
                local noPets = Instance.new("TextLabel")
                noPets.BackgroundTransparency = 1
                noPets.Size = UDim2.new(1, 0, 0, 30)
                noPets.Font = t4.body
                noPets.Text = "Pet data not exposed by the current egg directory"
                noPets.TextColor3 = t3.mute
                noPets.TextSize = 10
                noPets.TextXAlignment = Enum.TextXAlignment.Left
                noPets.Parent = petsFrame
            end
        end

        local function leRarityRank(r)
            local ranks = {common=1, uncommon=2, rare=3, epic=4, legendary=5, mythic=6, cosmic=7, secret=8, eternal=9, divine=10, titan=11}
            return ranks[string.lower(tostring(r or ""))] or 0
        end

        local function lePassRarity(egg)
            local mode = tostring(t9.LiveEggRarity or "Any")
            if mode == "Any" then return true end
            local cfg = egg.cfg or (Directory and egg.AssetCategory and Directory[egg.AssetCategory])
            local r = ""
            if type(cfg) == "table" then
                if type(cfg.Rarity) == "table" then
                    r = cfg.Rarity.DisplayName or cfg.Rarity.Name or cfg.Rarity._id or ""
                else
                    r = cfg.Rarity or cfg.RarityName or ""
                end
            end
            return leRarityRank(r) >= leRarityRank(mode)
        end

        local function lePassESP(egg)
            local filter = tostring(t9.ESPFilter or "All eggs")
            if filter == "All eggs" then return true end
            if filter == "Eggs matching my filters" then
                return v1215(egg, egg.cfg, false)
            end
            if filter == "Stolen target only" then
                local target = t216 and t216.target
                return target and target.rec and target.rec.Uid == egg.Uid
            end
            return true
        end

        local function leGetMutations(rec)
            local muts = {}
            if type(rec) ~= "table" then return muts end
            local src = rec.Mutations
            if type(src) ~= "table" and type(rec.ItemData) == "table" then src = rec.ItemData.Mutations end
            if type(src) == "table" then
                for _, m in ipairs(src) do
                    local sm = tostring(m)
                    if sm ~= "" then muts[#muts + 1] = sm end
                end
            elseif type(rec.Mutation) == "string" then
                muts[1] = rec.Mutation
            end
            return muts
        end

        local function leReadMyPlotRecords()
            local out = {}
            pcall(function()
                if u1103 and type(u1103.ReadOwnerEggs) == "function" then
                    local r = u1103.ReadOwnerEggs(LocalPlayer.UserId)
                    if type(r) == "table" then
            ...
            if #out == 0 then
                pcall(function()
                    if u1103 and type(u1103.ReadOwnedEggs) == "function" then
                        local r = u1103.ReadOwnedEggs()
                        if type(r) == "table" then
                            for _, ownerData in pairs(r) do
                                if type(ownerData) == "table" and tonumber(ownerData.OwnerUserId) == tonumber(LocalPlayer.UserId) then
                                    local records = ownerData.Records or ownerData.Eggs or ownerData.Items
        ...
        local function leCollectPlotEggs()
            local out = {}
            local seen = {}

            local function addRecord(rec)
                if type(rec) ~= "table" or seen[rec] then return end
                local owner = tonumber(rec.OwnerUserId or rec.ownerUserId or rec.OwnerId or rec.UserId)
                local mine = owner and owner == tonumber(LocalPlayer.UserId)
                local pos = v1209(rec)
                local isPlot = rec.Placement ~= nil
                    or (rec.Uid and (string.lower(tostring(rec.Uid)):find("plot",1,true)
                        or string.lower(tostring(rec.Uid)):find(":pen",1,true)
                        or string.lower(tostring(rec.Uid)):find("hatch",1,true)))
                    or (rec.State == "Slot" and mine)
                    or (mine and rec.State ~= "Dropped")

                if not mine and not isPlot then return end
                if not rec.AssetCategory then return end

                local cfg = Directory and Directory[rec.AssetCategory]
                if not cfg then return end
                seen[rec] = true

                local pets = {}
                pcall(function() leCollectPets(cfg, pets, nil, 0, rec.AssetCategory) end)
                table.sort(pets, function(a,b)
                    if (a.earn or 0) ~= (b.earn or 0) then return (a.earn or 0) > (b.earn or 0) end
                    return tostring(a.name) < tostring(b.name)
                end)

                local uniquePets, petSeen = {}, {}
                for _, pet in ipairs(pets) do
                    local key = string.lower(tostring(pet.name))
                    if not petSeen[key] then
                        petSeen[key] = true
                        pet.icon = leIcon(pet.cfg, pet.cat, pet.name)
                ...
                out[#out + 1] = {
                    ...
                    pets=uniquePets,
                    earn=v1193(rec,cfg),
                    mutations=leGetMutations(rec),
                    pos=pos
                }
            end

            -- Primeiro tenta a fonte específica do plot.
            pcall(function()
                if u1103 and type(u1103.ReadOwnerEggs) == "function" then
                    local r = u1103.ReadOwnerEggs(LocalPlayer.UserId)
                    if type(r) == "table" then
                        for _, v in pairs(r) do
                            if type(v) == "table" then
                                if v.Records or v.Eggs or v.Items then
                                    for _, rec in pairs(v.Records or v.Eggs or v.Items) do addRecord(rec) end
                                else
                                    addRecord(v)
                                end
                            end
            ...
            -- Fallback: usa os registros de campo e mantém somente os que são do plot/owner.
            pcall(function()
                local records = v1204(true)
                if type(records) == "table" then
                    for _, rec in ipairs(records) do addRecord(rec) end
                end
            end)

            table.sort(out, function(a,b)
                if (a.earn or 0) ~= (b.earn or 0) then return (a.earn or 0) > (b.earn or 0) end
                return tostring(a.cfg and (a.cfg.DisplayName or a.rec.AssetCategory) or a.rec.AssetCategory) < tostring(b.cfg and (b.cfg.DisplayName or b.rec.AssetCategory) or b.rec.AssetCategory)
            end)
            return out
        end

        -- Live: mostra somente o pet mais caro encontrado no servidor.
        local function leCollectBestServerPet()
            local best = nil
            local records = v1204(true)
            if type(records) ~= "table" then return nil end
            for _, egg in ipairs(records) do
                if type(egg) == "table" and egg.AssetCategory then
                    local cfg = Directory and Directory[egg.AssetCategory]
                    if cfg then
                        local pets = {}
                        pcall(function() leCollectPets(cfg, pets, nil, 0, egg.AssetCategory) end)
                        for _, pet in ipairs(pets) do
                            if type(pet) == "table" and type(pet.earn) == "number" then
                                if not best or pet.earn > best.earn then
                                    pet.icon = leIcon(pet.cfg, pet.cat, pet.name)
                                    pet.sourceEgg = cfg.DisplayName or egg.AssetCategory
                                    best = pet
                                end
                            end
                        end
        ...
        local function leRefreshTopPets()
            if not LiveEggsTopPets then return end
            for _, child in ipairs(LiveEggsTopPets:GetChildren()) do
                if not child:IsA("UIListLayout") then child:Destroy() end
            end

            local pet = leCollectBestServerPet()
            local row = Instance.new("Frame")
            row.LayoutOrder = 1
            row.Size = UDim2.new(1,-8,0,68)
            row.BackgroundColor3 = t3.card
            row.BorderSizePixel = 0
            row.Parent = LiveEggsTopPets
            leRound(row,10)
            leStroke(row,t3.line,1)
            ...
            icon.Size=UDim2.fromOffset(52,52)
            icon.ScaleType=Enum.ScaleType.Fit
            icon.Image=pet and (pet.icon or "") or ""
            icon.Parent=row

            local label=Instance.new("TextLabel")
            label.BackgroundTransparency=1
            label.Position=UDim2.fromOffset(70,7)
            label.Size=UDim2.new(1,-76,0,22)
            label.Font=t4.mid
            label.Text=pet and tostring(pet.name) or "Nenhum pet encontrado"
            label.TextColor3=t3.text
            label.TextSize=12
            ...
            sub.Position=UDim2.fromOffset(70,31)
            sub.Size=UDim2.new(1,-76,0,27)
            sub.Font=t4.body
            sub.Text=pet and ("BEST SERVER  •  "..leFmt(pet.earn).."/s  •  "..tostring(pet.sourceEgg or "egg")) or "Sem dados do servidor"
            sub.TextColor3=t3.accent
            sub.TextSize=9
        ...
        local leThiefHighlights = {}
        local thiefEspEnabled = true
        local liveThiefInfo = {player=nil, egg=nil, earn=0}

        local function leUpdateThiefESP()
            if thiefEspEnabled == false then return end
            ...
            if type(records) == "table" then
                for _,rec in ipairs(records) do
                    if type(rec)=="table" then
                        local uid = tonumber(rec.CarrierUserId)
                        local cfg = Directory and rec.AssetCategory and Directory[rec.AssetCategory]
                        local earn = cfg and tonumber(v1193(rec,cfg)) or 0
                        local state = string.lower(tostring(rec.State or ""))
                        local carried = uid and uid > 0 and (state == "carried" or state == "carrying" or state == "stolen" or rec.CarrierUserId ~= nil)
                        if carried and earn > bestEarn then
                            wanted = uid
                            wantedEgg = rec
                            bestEarn = earn
                        end
            ...
            liveThiefInfo.player = wanted and lePlayerForUserId(wanted) or nil
            liveThiefInfo.egg = wantedEgg
            liveThiefInfo.earn = math.max(0,bestEarn)

            for uid,h in pairs(leThiefHighlights) do
                if uid ~= wanted then
                    pcall(function() h:Destroy() end)
                    local plr=lePlayerForUserId(uid)
                    local char=plr and plr.Character
                    if char then pcall(function() char.GomesHubThiefTag:Destroy() end) end
                    leThiefHighlights[uid]=nil
                end
            end

            if wanted then
                local plr=lePlayerForUserId(wanted)
                local char=plr and plr.Character
                if char then
                    local h=leThiefHighlights[wanted]
                    if not h or not h.Parent then
                        h=Instance.new("Highlight")
                        h.Name="GomesHubThiefESP"
                        h.FillTransparency=.55
                        h.OutlineTransparency=0
                        h.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop
                        h.Parent=char
                        leThiefHighlights[wanted]=h
                    end
                    h.Adornee=char
                    h.FillColor=t3.accent
                    h.OutlineColor=t3.text

                    local bb = char:FindFirstChild("GomesHubThiefTag")
                    if not bb then
                        bb=Instance.new("BillboardGui")
                        bb.Name="GomesHubThiefTag"
                        bb.Size=UDim2.fromOffset(230,38)
                        ...
                        tx.Font=t4.mid
                        tx.TextSize=12
                        tx.TextColor3=t3.text
                        tx.TextStrokeTransparency=.35
                        tx.Parent=bb
                    end
                    local tx=bb:FindFirstChild("Label")
                    if tx then
                        tx.Text="THIEF  •  "..tostring(plr.Name)
                    end
                end
            end

            if LiveEggsThiefLabel then
                if wanted then
                    local eggCfg = wantedEgg and wantedEgg.AssetCategory and Directory and Directory[wantedEgg.AssetCategory]
                    local eggName = eggCfg and (eggCfg.DisplayName or wantedEgg.AssetCategory) or "Best Egg"
                    LiveEggsThiefLabel.Text = "ROUBANDO: "..tostring(liveThiefInfo.player and liveThiefInfo.player.Name or wanted).."  •  "..tostring(eggName).."  •  "..leFmt(bestEarn).."/s"
                else
                    LiveEggsThiefLabel.Text = "ROUBANDO: ninguém detectado"
                end
            end
        end

        local function leMakePlotRow(parent, item, order)
            local card = Instance.new("Frame")
            card.LayoutOrder = order
            card.Size = UDim2.new(1,-8,0,math.max(92, 68 + #item.pets*42))
            card.BackgroundColor3 = t3.card
            card.BorderSizePixel = 0
            card.Parent = parent
            leRound(card, 11)
            ...
            eggIcon.Position=UDim2.fromOffset(8,8)
            eggIcon.Size=UDim2.fromOffset(48,48)
            eggIcon.ScaleType=Enum.ScaleType.Fit
            eggIcon.Image=leIcon(item.cfg,item.rec.AssetCategory,item.cfg.DisplayName or item.rec.AssetCategory)
            eggIcon.Parent=card
            ...
            title.Font=t4.title
            title.Text=tostring(order)..". "..tostring(item.cfg.DisplayName or item.rec.AssetCategory)
            title.TextColor3=t3.text
            title.TextSize=12
            title.TextXAlignment=Enum.TextXAlignment.Left
            title.TextTruncate=Enum.TextTruncate.AtEnd
            ...
            local mut = #item.mutations > 0 and table.concat(item.mutations, " + ") or "Normal"
            local info=Instance.new("TextLabel")
            info.BackgroundTransparency=1
            info.Position=UDim2.fromOffset(64,27)
            info.Size=UDim2.new(1,-72,0,18)
            info.Font=t4.body
            info.Text=mut.."  ·  "..leFmt(item.earn).."/s"
            info.TextColor3=t3.accent
            info.TextSize=10
            info.TextXAlignment=Enum.TextXAlignment.Left
            info.TextTruncate=Enum.TextTruncate.AtEnd
            info.Parent=card
            ...
            local y=62
            for i,pet in ipairs(item.pets) do
                local row=Instance.new("Frame")
                ...
                pi.Size=UDim2.fromOffset(32,32)
                pi.Position=UDim2.fromOffset(0,3)
                pi.Image=pet.icon or ""
                pi.ScaleType=Enum.ScaleType.Fit
                pi.Parent=row
                local pn=Instance.new("TextLabel")
                pn.BackgroundTransparency=1
                pn.Position=UDim2.fromOffset(40,1)
                pn.Size=UDim2.new(1,-130,0,18)
                pn.Font=t4.mid
                pn.Text=tostring(pet.name)
                pn.TextColor3=t3.text
                pn.TextSize=10
                ...
                pm.Position=UDim2.fromOffset(40,19)
                pm.Size=UDim2.new(1,-130,0,16)
                pm.Font=t4.body
                pm.Text=tostring(pet.rarity or "?")..(pet.chance and (" · "..tostring(pet.chance)) or "")
                pm.TextColor3=t3.dim
                pm.TextSize=9
                pm.TextXAlignment=Enum.TextXAlignment.Left
                pm.Parent=row
                local pe=Instance.new("TextLabel")
                pe.BackgroundTransparency=1
                pe.AnchorPoint=Vector2.new(1,0.5)
                pe.Position=UDim2.new(1,-4,0.5,0)
                pe.Size=UDim2.fromOffset(84,20)
                pe.Font=t4.mid
                pe.Text=pet.earn>0 and (leFmt(pet.earn).."/s") or "—"
                pe.TextColor3=t3.accent
                pe.TextSize=9
                pe.TextXAlignment=Enum.TextXAlignment.Right
                pe.Parent=row
                y += 42
            end
        end

        local function leRefresh()
            if not LiveEggsPanel or not LiveEggsPanel.Parent then return end
            pcall(leUpdateThiefESP)
            if LiveEggsCurrentTab == "Eggs" then pcall(leRefreshTopPets) end
                ...
                if type(records) == "table" then
                    for _, egg in ipairs(records) do
                        if type(egg) == "table" and egg.Uid then
                            ...
                            if state == "Slot" or state == "Dropped" then
                                local cfg = Directory and egg.AssetCategory and Directory[egg.AssetCategory]
                                local pos = v1209(egg)
                                if pos and cfg then
                                    egg.cfg = cfg
                                    local useESP = t9.LiveEggUseESP == true
                                    local pass = lePassRarity(egg)
                                    if useESP then
                                        pass = pass and lePassESP(egg)
                                    end
                                    if pass then
                                        eggs[#eggs + 1] = egg
                                    end
            ...
            table.sort(eggs, function(a, b)
                local ea = v1193(a, Directory and a.AssetCategory and Directory[a.AssetCategory])
                local eb = v1193(b, Directory and b.AssetCategory and Directory[b.AssetCategory])
                if ea ~= eb then return ea > eb end
                return tostring(a.AssetCategory or "") < tostring(b.AssetCategory or "")
            end)
            ...
            for _, egg in ipairs(eggs) do
                ...
                if type(cfg) == "table" then
                    if type(cfg.Rarity) == "table" then
                        rar = cfg.Rarity.DisplayName or cfg.Rarity.Name or cfg.Rarity._id or ""
                    else
                        rar = cfg.Rarity or cfg.RarityName or ""
                    end
                end
                if leRarityRank(rar) >= leRarityRank("Mythic") then
                    highEgg = egg
                    break
                end
            end

            if highEgg and t9.LiveEggAutoOpen == true and not LiveEggsPanel.Visible then
                LiveEggsPanel.Visible = true
            end

            if highEgg and t9.LiveEggAlert == true then
                local now = os.clock()
                if now - (LiveEggsPanel:GetAttribute("LastHighEggAlert") or 0) > 5 then
                    LiveEggsPanel:SetAttribute("LastHighEggAlert", now)
                    if LiveEggsStatus then
                        local cfg = highEgg.cfg
                        local nm = cfg and (cfg.DisplayName or highEgg.AssetCategory) or highEgg.AssetCategory
                        LiveEggsStatus.Text = "★ " .. tostring(nm) .. " · Mythic+"
                    end
                end
            end
            ...
            for i, egg in ipairs(eggs) do
                leMakeEggRow(LiveEggsList, egg, i)
            end

            if LiveEggsPlotList then
                for _, child in ipairs(LiveEggsPlotList:GetChildren()) do
                    if not child:IsA("UIListLayout") then child:Destroy() end
                end
                local plotEggs = {}
                pcall(function() plotEggs = leCollectPlotEggs() end)
                for i,item in ipairs(plotEggs) do leMakePlotRow(LiveEggsPlotList,item,i) end
                if LiveEggsCurrentTab == "Plot" and LiveEggsStatus then
                    LiveEggsStatus.Text = tostring(#plotEggs) .. " plot eggs · updated now"
                end
            end

            if LiveEggsStatus and LiveEggsCurrentTab == "Eggs" then
                LiveEggsStatus.Text = tostring(#eggs) .. " live eggs · updated now"
            end
        end

        LiveEggsGui = Instance.new("Frame")
        LiveEggsGui.Name = "LiveEggsRoot"
        LiveEggsGui.BackgroundTransparency = 1
        LiveEggsGui.Size = UDim2.fromScale(1, 1)
        LiveEggsGui.ZIndex = 70
        LiveEggsGui.Parent = v98

        LiveEggsButton = Instance.new("TextButton")
        LiveEggsButton.Name = "LiveEggsButton"
        LiveEggsButton.AnchorPoint = Vector2.new(0, 1)
        LiveEggsButton.Position = UDim2.new(0, 18, 1, -18)
        LiveEggsButton.Size = UDim2.fromOffset(96, 34)
        LiveEggsButton.BackgroundColor3 = t3.rail
        LiveEggsButton.BorderSizePixel = 0
        LiveEggsButton.AutoButtonColor = false
        LiveEggsButton.Text = "🥚  GOMES LIVE"
        LiveEggsButton.TextColor3 = t3.text
        LiveEggsButton.TextSize = 11
        LiveEggsButton.Font = t4.mid
        LiveEggsButton.ZIndex = 71
        LiveEggsButton.Parent = LiveEggsGui
        leRound(LiveEggsButton, 10)
        leStroke(LiveEggsButton, t3.line, 1)

        LiveEggsPanel = Instance.new("Frame")
        LiveEggsPanel.Name = "LiveEggsPanel"
        LiveEggsPanel.AnchorPoint = Vector2.new(0, 1)
        LiveEggsPanel.Position = UDim2.new(0, 18, 1, -60)
        LiveEggsPanel.Size = UDim2.fromOffset(330, 405)
        LiveEggsPanel.BackgroundColor3 = t3.bg
        LiveEggsPanel.BorderSizePixel = 0
        LiveEggsPanel.Visible = false
        LiveEggsPanel.ZIndex = 72
        LiveEggsPanel.Parent = LiveEggsGui
        leRound(LiveEggsPanel, 14)
        leStroke(LiveEggsPanel, t3.line, 1)

        local head = Instance.new("Frame")
        head.BackgroundColor3 = t3.rail
        head.BorderSizePixel = 0
        head.Size = UDim2.new(1, 0, 0, 58)
        head.ZIndex = 73
        head.Parent = LiveEggsPanel
        leRound(head, 14)
        ...
        LiveEggsStatus = Instance.new("TextLabel")
        LiveEggsStatus.BackgroundTransparency = 1
        LiveEggsStatus.Position = UDim2.fromOffset(12, 30)
        LiveEggsStatus.Size = UDim2.new(1, -24, 0, 18)
        LiveEggsStatus.Font = t4.body
        LiveEggsStatus.Text = "loading..."
        LiveEggsStatus.TextColor3 = t3.dim
        LiveEggsStatus.TextSize = 10
        LiveEggsStatus.TextXAlignment = Enum.TextXAlignment.Left
        LiveEggsStatus.ZIndex = 74
        LiveEggsStatus.Parent = head

        local close = Instance.new("TextButton")
        close.AnchorPoint = Vector2.new(1, 0.5)
        close.Position = UDim2.new(1, -9, 0.5, 0)
        close.Size = UDim2.fromOffset(28, 28)
        close.BackgroundColor3 = t3.fill
        close.BorderSizePixel = 0
        close.AutoButtonColor = false
        close.Text = "×"
        close.TextColor3 = t3.text
        close.TextSize = 16
        close.Font = t4.mid
        close.ZIndex = 75
        close.Parent = head
        ...
        LiveEggsEggTab = Instance.new("TextButton")
        LiveEggsEggTab.Size = UDim2.fromOffset(70,24)
        LiveEggsEggTab.Position = UDim2.fromOffset(8,44)
        LiveEggsEggTab.BackgroundColor3 = t3.fill
        LiveEggsEggTab.BorderSizePixel = 0
        LiveEggsEggTab.Text = "Eggs"
        LiveEggsEggTab.TextColor3 = t3.text
        LiveEggsEggTab.TextSize = 10
        LiveEggsEggTab.Font = t4.mid
        LiveEggsEggTab.ZIndex = 75
        LiveEggsEggTab.Parent = LiveEggsPanel
        leRound(LiveEggsEggTab,7)

        LiveEggsPlotTab = LiveEggsEggTab:Clone()
        LiveEggsPlotTab.Name = "PlotTab"
        LiveEggsPlotTab.Position = UDim2.fromOffset(82,44)
        LiveEggsPlotTab.Text = "Plot"
        LiveEggsPlotTab.Parent = LiveEggsPanel

        local thiefEspButton = LiveEggsEggTab:Clone()
        thiefEspButton.Name = "ThiefESPButton"
        thiefEspButton.Position = UDim2.fromOffset(156,44)
        thiefEspButton.Size = UDim2.fromOffset(82,24)
        thiefEspButton.Text = "Thief ESP: ON"
        thiefEspButton.Parent = LiveEggsPanel
        thiefEspButton.MouseButton1Click:Connect(function()
            thiefEspEnabled = not thiefEspEnabled
            thiefEspButton.Text = thiefEspEnabled and "Thief ESP: ON" or "Thief ESP: OFF"
            if not thiefEspEnabled then
                for uid,h in pairs(leThiefHighlights) do
                    pcall(function() h:Destroy() end)
                    leThiefHighlights[uid]=nil
                end
        ...
        LiveEggsThiefLabel = Instance.new("TextLabel")
        LiveEggsThiefLabel.Name = "ThiefInfo"
        LiveEggsThiefLabel.Position = UDim2.fromOffset(8, 52)
        LiveEggsThiefLabel.Size = UDim2.new(1, -16, 0, 18)
        LiveEggsThiefLabel.BackgroundTransparency = 1
        LiveEggsThiefLabel.Font = t4.mid
        LiveEggsThiefLabel.Text = "ROUBANDO: ninguém detectado"
        LiveEggsThiefLabel.TextColor3 = t3.text
        LiveEggsThiefLabel.TextSize = 9
        LiveEggsThiefLabel.TextXAlignment = Enum.TextXAlignment.Left
        LiveEggsThiefLabel.TextTruncate = Enum.TextTruncate.AtEnd
        LiveEggsThiefLabel.ZIndex = 74
        LiveEggsThiefLabel.Parent = LiveEggsPanel

        LiveEggsTopPets = Instance.new("ScrollingFrame")
        LiveEggsTopPets.Name = "TopServerPets"
        LiveEggsTopPets.Position = UDim2.fromOffset(8,72)
        LiveEggsTopPets.Size = UDim2.new(1,-16,0,82)
        LiveEggsTopPets.BackgroundTransparency=1
        LiveEggsTopPets.BorderSizePixel=0
        LiveEggsTopPets.ScrollBarThickness=3
        LiveEggsTopPets.ScrollBarImageColor3=t3.accent
        LiveEggsTopPets.CanvasSize=UDim2.new()
        LiveEggsTopPets.AutomaticCanvasSize=Enum.AutomaticSize.Y
        LiveEggsTopPets.ZIndex=73
        LiveEggsTopPets.Parent=LiveEggsPanel
        local topLayout=Instance.new("UIListLayout")
        topLayout.Padding=UDim.new(0,3)
        topLayout.SortOrder=Enum.SortOrder.LayoutOrder
        topLayout.Parent=LiveEggsTopPets

        LiveEggsList = Instance.new("ScrollingFrame")
        LiveEggsList.Name = "List"
        LiveEggsList.Position = UDim2.fromOffset(8, 164)
        LiveEggsList.Size = UDim2.new(1, -16, 1, -166)
        LiveEggsList.BackgroundTransparency = 1
        LiveEggsList.BorderSizePixel = 0
        LiveEggsList.ScrollBarThickness = 3
        LiveEggsList.ScrollBarImageColor3 = t3.accent
        LiveEggsList.CanvasSize = UDim2.new()
        LiveEggsList.AutomaticCanvasSize = Enum.AutomaticSize.Y
        LiveEggsList.ZIndex = 73
        LiveEggsList.Parent = LiveEggsPanel

        LiveEggsPlotList = LiveEggsList:Clone()
        LiveEggsPlotList.Name = "PlotList"
        LiveEggsPlotList.Position = UDim2.fromOffset(8,72)
        LiveEggsPlotList.Size = UDim2.new(1,-16,1,-74)
        LiveEggsPlotList.Parent = LiveEggsPanel
        LiveEggsPlotList.Visible = false
        for _, child in ipairs(LiveEggsPlotList:GetChildren()) do child:Destroy() end

        local listLayout = Instance.new("UIListLayout")
        listLayout.Padding = UDim.new(0, 6)
        listLayout.SortOrder = Enum.SortOrder.LayoutOrder
        listLayout.Parent = LiveEggsList
        local plotLayout = Instance.new("UIListLayout")
        plotLayout.Padding = UDim.new(0,6)
        plotLayout.SortOrder = Enum.SortOrder.LayoutOrder
        plotLayout.Parent = LiveEggsPlotList

        -- Arraste pelo cabeçalho inteiro.
        do
            ...
            head.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1
                    ...
                    startPos = LiveEggsPanel.Position
            ...
            UserInputService.InputChanged:Connect(function(input)
                ...
                LiveEggsPanel.Position = UDim2.new(
                    startPos.X.Scale,
        ...
        local liveOpen = false
        local function leToggle(state)
            liveOpen = state == nil and not liveOpen or state
            LiveEggsPanel.Visible = liveOpen
            if liveOpen then
                task.spawn(leRefresh)
            end
        end

        local function leSetTab(tab)
            LiveEggsCurrentTab = tab
            if LiveEggsList then LiveEggsList.Visible = tab == "Eggs" end
            if LiveEggsTopPets then LiveEggsTopPets.Visible = tab == "Eggs" end
            if LiveEggsPlotList then LiveEggsPlotList.Visible = tab == "Plot" end
            if LiveEggsStatus then LiveEggsStatus.Text = tab == "Eggs" and "loading eggs..." or "loading plot..." end
            pcall(leRefresh)
        end
        LiveEggsEggTab.MouseButton1Click:Connect(function() leSetTab("Eggs") end)
        LiveEggsPlotTab.MouseButton1Click:Connect(function() leSetTab("Plot") end)

        LiveEggsButton.MouseButton1Click:Connect(function()
            leToggle()
        end)
        ...
        task.spawn(function()
            while LiveEggsGui and LiveEggsGui.Parent do
                pcall(leUpdateThiefESP)
                if liveOpen then
                    pcall(leRefresh)
                end
            ...
            for _,h in pairs(leThiefHighlights) do pcall(function() h:Destroy() end) end
        end)

        -- Expõe a função no dump global do hub sem alterar as funções existentes.
    ...
    v1102("boot", "game logic attached · GomesDump() in F9")
end)()
