...
(function()
    ...
    do
        ...
        local function lePetRankName(pet)
            local r = string.lower(tostring(pet and pet.rarity or ""))
            if r:find("cosmic",1,true) then return "Cosmic" end
            if r:find("secret",1,true) then return "Secret" end
            if r:find("eternal",1,true) then return "Eternal" end
            if r:find("divine",1,true) then return "Divine" end
            return nil
        end

        local function leCollectTopServerPets()
            local best = {Cosmic=nil, Secret=nil, Eternal=nil, Divine=nil}
            local records = v1204(true)
            if type(records) ~= "table" then return best end
            for _, egg in ipairs(records) do
                if type(egg) == "table" and egg.AssetCategory then
                    ...
                    if cfg then
                        ...
                        for _, pet in ipairs(pets) do
                            local rank = lePetRankName(pet)
                            if rank then
                                local cur = best[rank]
                                if not cur or (pet.earn or 0) > (cur.earn or 0) then
                                    pet.icon = leIcon(pet.cfg, pet.cat, pet.name)
        ...
        local function leRefreshTopPets()
            ...
            local best = leCollectTopServerPets()
            local order = 0
            for _, rarity in ipairs({"Cosmic","Secret","Eternal","Divine"}) do
                local pet = best[rarity]
                order += 1
                local row = Instance.new("Frame")
                row.LayoutOrder=order; row.Size=UDim2.new(1,-8,0,52); row.BackgroundColor3=t3.card; row.BorderSizePixel=0; row.Parent=LiveEggsTopPets
                leRound(row,9); leStroke(row,t3.line,1)
                local icon=Instance.new("ImageLabel"); icon.BackgroundTransparency=1; icon.Position=UDim2.fromOffset(7,7); icon.Size=UDim2.fromOffset(38,38); icon.ScaleType=Enum.ScaleType.Fit; icon.Image=pet and (pet.icon or "") or ""; icon.Parent=row
                local label=Instance.new("TextLabel"); label.BackgroundTransparency=1; label.Position=UDim2.fromOffset(53,5); label.Size=UDim2.new(1,-50,0,18); label.Font=t4.mid; label.Text=rarity..": "..(pet and tostring(pet.name) or "não encontrado"); label.TextColor3=t3.text; label.TextSize=10; label.TextXAlignment=Enum.TextXAlignment.Left; label.Parent=row
                local sub=Instance.new("TextLabel"); sub.BackgroundTransparency=1; sub.Position=UDim2.fromOffset(53,25); sub.Size=UDim2.new(1,-50,0,15); sub.Font=t4.body; sub.Text=pet and (leFmt(pet.earn).."/s · "..tostring(pet.sourceEgg or "server")) or "sem dados"; sub.TextColor3=t3.dim; sub.TextSize=9; sub.TextXAlignment=Enum.TextXAlignment.Left; sub.Parent=row
            end
        end

        local function lePlayerForUserId(uid)
            uid=tonumber(uid); if not uid then return nil end
            for _,plr in ipairs(Players:GetPlayers()) do if tonumber(plr.UserId)==uid then return plr end end
            return nil
        end
        ...
        local function leUpdateThiefESP()
            ...
            if type(records)=="table" then
                for _,rec in ipairs(records) do
                    if type(rec)=="table" and (tostring(rec.State)=="Carried" or tostring(rec.State)=="Carrying" or tostring(rec.State)=="Stolen") then
                        local uid=tonumber(rec.CarrierUserId)
                        local cfg=Directory and rec.AssetCategory and Directory[rec.AssetCategory]
                        local earn=cfg and v1193(rec,cfg) or 0
                        if uid and uid ~= tonumber(LocalPlayer.UserId) and earn > bestEarn then wanted=uid; bestEarn=earn end
                    end
                end
            end
            ...
            if wanted then
                ...
                if char then
                    ...
                    if not h or not h.Parent then
                        h=Instance.new("Highlight"); h.Name="GomesHubThiefESP"; h.FillTransparency=.55; h.OutlineTransparency=0; h.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; h.Parent=char; leThiefHighlights[wanted]=h
                    end
                    h.Adornee=char; h.FillColor=t3.accent; h.OutlineColor=t3.text
                    ...
                    if tx then tx.Text="THIEF  •  "..tostring(plr.Name) end
                end
            end
        end
        ...
        LiveEggsTopPets.Size = UDim2.new(1,-16,0,190)
        ...
        LiveEggsList.Position = UDim2.fromOffset(8, 268)
        LiveEggsList.Size = UDim2.new(1, -16, 1, -270)
